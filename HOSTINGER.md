...
- **Unattended daily:** Hostinger cron at **6:00 AM IST** (`30 0 * * *` UTC) → `POST /api/erp-sync/daily` (fetch + server AI audit + cron auto-publish). Plus continue every 10 min (`*/10 * * * *` → `erp-sync/continue`, idle no-op) and keep-alive every 1 min (`*/1 * * * *` → **production** `erp-sync/keepalive`, not `/dev/api`). UI Save does not schedule pings. Super User only. See **[ERP-SYNC.md](./ERP-SYNC.md)**.
