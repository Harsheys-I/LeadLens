...
async function requestAudit(apiKey, settings, leads, signal) {
  ...
  const data=await response.json(); const content=data.choices?.[0]?.message?.content; if(!content) throw new Error("OpenAI returned no audit content.");
...
export async function auditBatch(apiKey, settings, batch, signal, log) {
  ...
  for(let attempt=1;attempt<=3;attempt++){ try{ result=await requestAudit(apiKey,settings,batch,signal); break; }catch(e){ if(e.name==="AbortError") throw e; lastError=e; log(`Attempt ${attempt} failed: ${e.message}`,"error"); if(attempt<3) await new Promise(r=>setTimeout(r,attempt*1500)); } }
  if(!result) throw lastError;
  const byId=new Map(result.map(r=>[clean(r.lead_id),r])); let missing=batch.filter(l=>!byId.has(l.lead_id));
  if(missing.length){ log(`Model omitted ${missing.length} lead(s); retrying only those leads.`,"warn"); const recovered=await requestAudit(apiKey,settings,missing,signal); recovered.forEach(r=>byId.set(clean(r.lead_id),r)); missing=batch.filter(l=>!byId.has(l.lead_id)); }
